...
class openvpnOVPN extends eqLogic {
	/*     * *************************Attributs****************************** */
	...
	public static function dependancy_install() {
		...
		return array('script' => dirname(__FILE__) . '/../../resources/install_#stype#.sh ' . jeedom::getTmpFolder('openvpnOVPN') . '/dependance', 'log' => log::getPathToLog(__CLASS__ . '_update'));
	}
	...
	public static function cron5() {
		foreach (self::byType('openvpnOVPN') as $eqLogic) {
			try {
	...
	private function writeConfig() {
		...
		$replace = array(
			...
			'#auth_path#' => jeedom::getTmpFolder('openvpnOVPN') . '/openvpnOVPN_auth_' . $this->getConfiguration('key') . '.conf',
		);
		
		if ($this->getConfiguration('auth_mode') == 'password') {
			$replace['#authentification#'] = 'auth-user-pass ' . jeedom::getTmpFolder('openvpnOVPN') . '/openvpnOVPN_auth_' . $this->getConfiguration('key') . '.conf';
			file_put_contents(jeedom::getTmpFolder('openvpnOVPN') . '/openvpnOVPN_auth_' . $this->getConfiguration('key') . '.conf', trim($this->getConfiguration('username')) . "\n" . trim($this->getConfiguration('password')));
		} else {
		...
		$config = str_replace(array_keys($replace), $replace, file_get_contents(dirname(__FILE__) . '/../config/openvpnOVPN.client.tmpl.ovpn'));
		if (trim($this->getConfiguration('additionalVpnParameters')) != '') {
			$config .= "\n\n" . $this->getConfiguration('additionalVpnParameters');
		}
		file_put_contents(jeedom::getTmpFolder('openvpnOVPN') . '/openvpnOVPN_' . $this->getId() . '.ovpn', $config);
	}
	...
	public function start_openvpnOVPN() {
		...
		if (trim($this->getConfiguration('optionsAfterStart')) != '') {
			...
			log::add('openvpnOVPN','debug','Exec post start cmd : '.$cmd);
			shell_exec($cmd);
		}
...
class openvpnOVPNCmd extends cmd {
	/*     * *************************Attributs****************************** */
