...
function printEqLogic(_eqLogic){
  $('#bt_uploadCaCrt').fileupload({
    ...
    url: 'plugins/openvpnOVPN/core/ajax/openvpnOVPN.ajax.php?action=uploadCaCrt&type=ca&id=' + _eqLogic.id+'&jeedom_token='+JEEDOM_AJAX_TOKEN,
    dataType: 'json',
  ...
  $('#bt_uploadCaCrtClient').fileupload({
    replaceFileInput: false,
    url: 'plugins/openvpnOVPN/core/ajax/openvpnOVPN.ajax.php?action=uploadCaCrt&type=caClient&id=' + _eqLogic.id+'&jeedom_token='+JEEDOM_AJAX_TOKEN,
    dataType: 'json',
    done: function (e, data) {
      if (data.result.state != 'ok') {
        $('#div_alert').showAlert({message: data.result.result, level: 'danger'});
  ...
  $('#bt_uploadCaKeyClient').fileupload({
    replaceFileInput: false,
    url: 'plugins/openvpnOVPN/core/ajax/openvpnOVPN.ajax.php?action=uploadCaCrt&type=keyClient&id=' + _eqLogic.id+'&jeedom_token='+JEEDOM_AJAX_TOKEN,
    dataType: 'json',
    done: function (e, data) {
      if (data.result.state != 'ok') {
        $('#div_alert').showAlert({message: data.result.result, level: 'danger'});
